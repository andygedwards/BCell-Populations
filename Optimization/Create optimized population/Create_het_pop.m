% Andy Edwards (Feb 25 August 2024)
...
exp_name = "Original";
base_paramFile = @Riz2014_init_parameters_INa_low;

frac_high = 1.00;  % Fraction of High INa cells
...
if ~isfolder(sprintf('%s/%.0f_%.2f/',exp_name,num,frac_high))
    mkdir(sprintf('%s/%.0f_%.2f/',exp_name,num,frac_high))
end

scaling_name = sprintf('%s/%.0f_%.2f/modParam_scaling_%.0f_%.2f.mat',exp_name,num,frac_high,num, frac_high); save(scaling_name, 'modParam_scaling');
stdev_name = sprintf('%s/%.0f_%.2f/modParam_stdev_%.0f_%.2f.mat',exp_name,num,frac_high, num, frac_high); save(stdev_name, 'modParam_stdev');
param_name = sprintf('%s/%.0f_%.2f/modParam_names_%.0f_%.2f.mat',exp_name,num,frac_high, num, frac_high); save(param_name, 'modParam_names');
peak_INa_name = sprintf('%s/%.0f_%.2f/peak_INa_%.0f_%.2f.mat',exp_name,num,frac_high, num, frac_high); save(peak_INa_name, 'peak_INa');
v_half_act_name = sprintf('%s/%.0f_%.2f/v_half_act_%.0f_%.2f.mat',exp_name,num,frac_high, num, frac_high); save(v_half_act_name, 'v_half_act');
peak_ICa_name = sprintf('%s/%.0f_%.2f/peak_ICa_%.0f_%.2f.mat',exp_name,num,frac_high, num, frac_high); save(peak_ICa_name, 'peak_ICa');
late_ICa_name = sprintf('%s/%.0f_%.2f/late_ICa_%.0f_%.2f.mat',exp_name,num,frac_high, num, frac_high); save(late_ICa_name, 'late_ICa');
EE_name = sprintf('%s/%.0f_%.2f/EE_%.0f_%.2f.mat',exp_name,num,frac_high, num, frac_high); save(EE_name, 'EE');
TE_name = sprintf('%s/%.0f_%.2f/TE_%.0f_%.2f.mat',exp_name,num,frac_high, num, frac_high); save(TE_name, 'TE');
v_half_name = sprintf('%s/%.0f_%.2f/v_half_%.0f_%.2f.mat',exp_name,num,frac_high, num, frac_high); save(v_half_name, 'v_half');
frac_high_name = sprintf('%s/%.0f_%.2f/frac_high_%.0f_%.2f.mat',exp_name,num,frac_high, num, frac_high); save(frac_high_name, 'frac_high');

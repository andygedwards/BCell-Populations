function [Total_Error,EE_raw_error, TE_raw_error, early_ICa_raw_error, late_ICa_raw_error, peak_INa_raw_error, vhalf_raw_error] = Cost_Function_Na_multi_out(nTrials, P)
...
% Code last modified: Andy (October 2024)
    ...
    st_dev_vector(5) = 0.22; % g_CaL stdev (direct from experiments)
    st_dev_vector(6) = 0.59; % g_CaPQ stdev (direct from experiments)
    st_dev_vector(7) = 0.40; % g_CaT stdev (direct from experiments)
    ...
    modParam_stdev_noNa = st_dev_vector(1:8); % coefficients of variation for parameters that are not involved in either form of INa
    INa_high_modParam_stdev = st_dev_vector(9:12); % coefficients of variation for INa,high parameters 
    INa_low_modParam_stdev = st_dev_vector(13:16); % coefficients of variation for INa,low parameters

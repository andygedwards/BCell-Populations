function [modParam_names,modParam_inds,plot_names,plot_units] = select_plot_params_verbose(param_selection, param_names, param_inds)
      plot_names = {""};
      plot_units = {""};
      ...
      index_plot_names = {'V_{GKmax}', 'K_{GK}', 'V_{PFKmax}', 'K_{PFK}', 'h_{PFK}', 'V_{GAPDHmax}', 'g_{Kv}', 'g_{BK}', 'g_{CaL}', 'g_{CaPQ}', 'g_{CaT}', 'g_{KATP}', 'g_{hERG}', 'V_{hNa,high}', 'n_{hNa,high}', 'g_{Na,high}', 'V_{mNa,high}', 'V_{hNa,low}', 'n_{hNa,low}', 'g_{Na,low}', 'V_{mNa,low}'}; 

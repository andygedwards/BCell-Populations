% Code last modified: Andy Edwards (28 August 2024)
...
cd ..
home = cd;
addpath(strcat(home,'/Generate Figs/Accessory scripts'))
addpath(strcat(home,'/Baseline models/Riz_2014_INa_low'))
addpath(strcat(home,'/Voltage clamp'))

frac_high = 0.21;
nTrials = 3000;  %Number of cells in final population
%%
%Final populations
load(sprintf('Create Population/%.0f_%.2f/modParam_scaling_%.0f_%.2f%.mat',nTrials,frac_high,nTrials,frac_high))
...
%         '\n \x03BC = %.2f; \x03C3 = %.2f' ...
    ...
    if iModParam > (nModParams-6)
        xlabel('Fraction of default model value')
    end
    grid on;
end

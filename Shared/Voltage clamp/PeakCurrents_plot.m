function [peak_INa, v_half_act, vha_r2, peak_ICa, late_ICa, T, Y] = PeakCurrents_plot(parameters, parameter_names, states, state_names, rhs_model, colour)
    ...
    try
        outs = analyze_v_clamp_plotting(Y,T,parameters,state_names, cycle_length, prepulse_length, end_length, rhs_model, v_clamp_amps, v_clamp_times, R_clamp, current, plotting, colour);
        peak_INa = outs{1};
        v_half_act = outs{2};
        vha_r2 = outs{3};
        peak_ICa = outs{4};

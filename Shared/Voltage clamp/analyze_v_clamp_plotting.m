function [outs] = analyze_v_clamp_plotting(Y, T, parameters, state_names, CL, PP, EL, model, varargin)
...
%         end
        ...
        if func2str(model)=="Riz2014_rhs_INa_low"
            ...
            Sweeps_I_Na{i,:} = Sweep_I_Na_low+Sweep_I_Na_high;
        else
...
if ~isempty(varargin)
    switch current
        case 'exocytosis'
            ...
            subplot(2,1,1), plot(T,V,'k')
            xlabel('time (ms)')
            ylabel('V_{m} (mV)')
            subplot(2,1,2), plot(T,Ca_c,'Color','k','LineStyle',line,'LineWidth',2), hold on
            xlabel('time (ms)')
            ...
            try
                ...
                for i = 1:length(V)
                    ...
                    if plotting 
                        ...
                        subplot(2,2,1), V_h = plot(Sweeps_T{i}, Sweeps_V{i},'Color', colour, 'linewidth',lw); hold on                  
                        subplot(2,2,2), I_h = plot(Sweeps_T{i}, Sweeps_I{i},'Color', colour, 'linewidth',lw,'LineStyle', line); hold on 
                        subplot(2,2,3), INa_h = plot(Sweeps_T{i}, Sweeps_I_Na{i},'Color', colour, 'linewidth',lw,'LineStyle', line); hold on 
                    end
                end
                subplot(2,2,1), ylabel('V_{m} (mV)'), xlim([400,600])
                subplot(2,2,2), ylabel('I_{total} (A/F)'), xlabel('time (ms)'), xlim([400,600])
                subplot(2,2,3), ylabel('I_{Na} (A/F)'), xlabel('time (ms)'), xlim([400,600])
            ...
            try
                ...
                for i = 1:length(V)
                    ...
                    if plotting
                        ...
                        subplot(2,2,1), plot(Sweeps_T{i}, Sweeps_V{i},'linewidth',lw, 'LineStyle',line, color=colour), hold on
                        subplot(2,2,2), plot(Sweeps_T{i}, Sweeps_I_CaL{i},'linewidth',lw, 'LineStyle', line, color=colour), hold on
                        subplot(2,2,3), plot(Sweeps_T{i}, Sweeps_I_CaPQ{i},'linewidth',lw, 'LineStyle', line, color=colour), hold on
                        % subplot(2,2,3), plot(Sweeps_T{i}, Sweeps_I_CaT{i},'r')
                        % if func2str(model)=="Riz2014_rhs_INa_low"
                        %     subplot(2,2,4), plot(Sweeps_T{i}, Sweeps_I_Na_high{i},colour), hold on
                        %     subplot(2,2,4), plot(Sweeps_T{i}, Sweeps_I_Na_low{i},colour)
                        % else
                        %     subplot(2,2,4), plot(Sweeps_T{i}, Sweeps_I_Na{i},colour), hold on
                        % end
                        subplot(2,2,1), ylabel('V_{m} (mV)'), xlim([900,1200])
                        subplot(2,2,2), ylabel('I_{CaL} (pA/pF)'), xlim([900,1200])
                        subplot(2,2,3), ylabel('I_{CaPQ} (pA/pF)'), xlim([900,1200])
